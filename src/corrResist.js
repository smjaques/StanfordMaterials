...
$w.onReady(function () {
	...
	$w("#SortText").text = "Sorted by: Corrosion Resistance";
});

$w.onReady(() => {
	let filter = wixData.filter();
	filter = filter.eq("corrosionResistance01", true);
	...
	$w('#clear').onClick(()=> {
		...
		$w('#sortby').value = "Sort By";
		$w('#sortby').resetValidityIndication();
		filter = filter.eq("corrosionResistance01", true);
		$w('#dataset1').setFilter(filter).then(count);
	})
	$w('#sortby').onChange(()=> {
	...
	$w("#table1").onRowSelect( (event) => {
		...
		session.setItem('page', '/corrosion-resistance');
	...
	function search() {
		...
		let weldability = $w('#weldability').value;
		...
		if (weldability && weldability !== 'all'){
			let lower = Number(weldability) - 1;
			let upper = Number(weldability) + 1;
